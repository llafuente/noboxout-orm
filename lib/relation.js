    ...
    Relation = __class("norm/relation", {
        ...
        applyRightReverse: function (query) {
            var r_tbl = this.root.$table,
                ...
                alias = this.property,
                left = [query.alias, r_tbl.prefix + this.refColumn],
        ...
        applyRight: function (query) {
            var r_tbl = this.root.$table,
                t_tbl = this.reference.$table,
                alias = this.property,
                left = [query.alias, r_tbl.prefix + this.foreignKey],
        ...
        applyLeftReverse: function (query) {
            ...
            var r_tbl = this.root.$table,
                ...
                left = [query.alias, t_tbl.prefix + this.foreignKey],
        ...
        applyLeft: function (query) {
            ...
            var r_tbl = this.root.$table,
                t_tbl = this.reference.$table,
                alias = this.refProperty,
                left = [query.alias, t_tbl.prefix + this.refColumn],
        ...
        storeOneToOne: function (entity, obj, connection, callback, retried) {
            var subentity,
                retry = function (err) {
                    if (err) {
                        return callback && callback(err, entity);
                    }
                    this.storeOneToOne(entity, obj, connection, callback, 1);
                }.bind(this);
            ...
            if (subentity !== null) {
                ...
                if (subentity[this.refColumn] === null) {
                    ...
                    subentity.$store().exec(connection, retry);
        ...
        storeOneToMany: function (entity, obj, connection, callback, __retried) {
            var max,
                i,
                subentity_list,
                subentity,
            ...
            if (max !== 0) {
                for (i = 0; i < max; ++i) {
                    ...
                    if (!object.empty(changes)) {
                        return subentity.$store().exec(connection, function () {
                            subentity.$db[self.foreignKey] = subentity[self.foreignKey];
                            self.storeOneToMany(entity, obj, connection, callback, 1);
                        });
                    }
                }
            }
        ...
        store: function (entity, obj, connection, callback) {
            ...
            case MANY_TO_ONE:
                ...
                if (this.isRoot(entity)) {
                    return this.storeOneToOne(entity, obj, connection, callback);
                }
                break;
            case ONE_TO_MANY:
                // only reference has FK
                if (this.isRoot(entity)) {
                    // it's previously stored
                    if (entity.$pk() !== null) {
                        return this.storeOneToMany(entity, obj, connection, callback);
                    }

                    // wire a post hook
                    entity.addEvent("hook:post:store", function (hook_callback) {
                        var subentity_list = entity[self.property],
                            i,
                            max = subentity_list.length;

                        if (!max) {
                            return hook_callback && hook_callback();
                        }

                        for (i = 0; i < max; ++i) {
                            subentity_list[i][self.foreignKey] = entity[self.refColumn];
                        }

                        self.storeOneToMany(entity, obj, connection, hook_callback);
                    });
                }
            }
        ...
        "static OneToOne": function (root_entity, target_entity, options) {
            ...
            root_entity.$column(rel.foreignKey, type);
        ...
        "static OneToMany": function (root_entity, target_entity, options) {
            ...
            target_entity.$column(rel.foreignKey, type);
            ...
            /*
            // handle indexes
            for (var i in root_entity) {
                console.log(i, root_entity[i]);
            }
            process.exit();
            var roots = root_entity.$self;
            */
            if (root_entity.$primaryKey() !== rel.foreignKey && !root_entity.$hasIndex([rel.foreignKey])) {
                root_entity.$index([rel.foreignKey]);
            }
