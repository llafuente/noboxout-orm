(function () {
    "use strict";

    var object = require("object-enhancements"),
        array = require("array-enhancements"),
        util = require("util"),
        Relation = require("./relation.js"),
        DBA = require("./dba.js"),
        norm = require("./norm.js"),
        $ = require("node-class"),
        __class = $.class;

    //$.debug = false;

    function store_related(con, relations, entity, obj, callback) {
        var relation,
            i,
            max = relations.length,
            retry = function () {
                store_related(con, relations, entity, obj, callback);
            };

        for (i = 0; i < max; ++i) {
            relation = relations[i];
            if (relation.hasForeignKey(entity)) {
                // true if there is work to be done
                if (relation.store(entity, obj, retry)) {
                    return;
                }
            }
        }
        store_entity_cb(con, entity, obj, callback);
    }

    function store_entity_cb(con, entity, obj, callback) {
        var table_data = entity.$self.$table;
        // check if obj is valid
        // any key is a FK ?

        if (table_data.prefix.length) {
            obj = object.prefixKeys(obj, table_data.prefix);
        }

        if (table_data.timestamps) {
            if (entity[table_data.primaryKey]) {
                obj[table_data.prefix + table_data.updatedAt] = new Date();
            } else {
                obj[table_data.prefix + table_data.createdAt] = new Date();
            }
        }

        var sql = DBA.store(table_data.tableName, table_data.prefix + table_data.primaryKey, obj);

        con.query(sql, function (err, result) {
            if (err) {
                throw err;
            }

            if (!entity[table_data.primaryKey]) {
                entity[table_data.primaryKey] = result.insertId;
            }

            entity.emit("saved");

            callback && callback(err, result);
        });
    }

    function store_entity(con, entity, callback) {

        var table_data = entity.$self.$table,
            relations = table_data.relations,
            //obj = object.extract(entity, table_data.fields);
            obj = entity.$changes();

        if (entity[table_data.primaryKey]) {
            obj[table_data.primaryKey] = entity[table_data.primaryKey];
        }

        norm.verbose("*********************");
        norm.verbose("store_related");

        store_related(con, relations, entity, obj, callback);
    }

    function export_type(prefix, field_name, options, pk) {
        var field = "    `" + prefix + field_name + "` " + options.$dbtype.toUpperCase();

        switch (options.$dbtype) {
            case "integer":
                field += "(" + options.length + ")";
                if (options.zerofill) {
                    field += " ZEROFILL";
                }
                if (options.unsigned) {
                    field += " UNSIGNED";
                }
                break;
            case "varchar":
                field += "(" + options.length + ")";
                break;

        }
        if (pk === field_name) {
            field += " AUTO_INCREMENT";
        }
        if (options.default) {
            field += " DEFAULT " + options.default;
        }
        /*
        if (options.charset) {
            field += " CHARACTER SET " + options.charset;
        }
        if (options.collation) {
            field += " COLLATE " + options.collation;
        }
        */

        return field;
    }

    __class("Norm/Entity", {
        extends: ["Events"],

        connection: null,

        initialize: function () {
            // set to false because
            this.__parent();
            this.$dirty = false;
            this.$constructor = false;
        },
        $changes: function () {
            norm.verbose(this.$data);
            norm.verbose(this.$db);
            return object.diff(this.$data, this.$db);
        },
        setConnection: function (con) {
            this.connection = con;
        },
        $pk: function () {
            var t = this.$self.$table;
            return this[t.primaryKey];
        },
        $store: function (con, callback) {

            switch(arguments.length) {
            case 1:
                callback = con;
                break;
            case 2:
                this.connection = con;
                break;
            }

            if (!this.connection) {
                throw new Error("connetion not specified");
            }

            store_entity(this.connection, this, callback);
        },
        //inspect: function() {
        //    return object.extract(this, this.$self.properties);
        //},
        "static $create": function (con) {
            var x = new this();
            if (con) {
                x.setConnection(con);
            }
            return x;
        },
        "static $delete": function (con, id, callback) {
            var sql = DBA.delete(this.$table)
            con.query(sql, [id], callback);
        },
        "static $from": function (result, nested, root, con) {
            nested = nested || false;
            root = root || false;

            var t,
                entity,
                i,
                max,
                rel,
                subentity,
                ret;

            if (!nested) {
                norm.verbose(result);
                if (!result || result[this.$table.prefix + this.$table.primaryKey] === null) {
                    return null;
                }

                if (this.$table.prefix.length) {
                    ret = new this(object.remPrefixKeys(result, this.$table.prefix));
                    ret.setConnection(con);

                    return ret;
                }

                ret = new this(result);
                ret.setConnection(con);

                return ret;
            }

            t = this.$table;
            entity = this.$from(result[root ? "root" : t.tableName], false, false, con);
            entity.setConnection(con);

            for (i = 0, max = t.relations.length; i < max; ++i) {
                rel = t.relations[i];

                switch (rel.type) {
                case "OneToMany":

                    norm.verbose(entity[rel.property]);

                    if (Array.isArray(result[rel.property])) {
                        for (i = 0, max = result[rel.property].length; i < max; ++i) {
                            subentity = rel.reference.$from(result[rel.property][i], false, false, con);
                            if (subentity) {
                                entity[rel.property].push(subentity);
                            }
                        }
                    } else {
                        subentity = rel.reference.$from(result[rel.property], false, false, con);
                        if (subentity) {
                            entity[rel.property].push(subentity);
                        }
                    }
                    break;
                case "OneToOne":
                    if (rel.isRoot(this)) {
                        if (Array.isArray(result[rel.property])) {
                            result[rel.property] = result[rel.property][0];
                        }
                        entity[rel.property] =  rel.reference.$from(result[rel.property], false, false, con);
                    } else {
                        if (Array.isArray(result[rel.property])) {
                            result[rel.refProperty] = result[rel.refProperty][0];
                        }
                        entity[rel.refProperty] =  rel.root.$from(result[rel.refProperty], false, false, con);
                    }
                    break;
                }
            }

            return entity;
        },
        "static $get": function (con, pkval, options, callback) {
            if (con.$class !== "norm/connection") {
                throw new Error("invalid connection argument");
            }
            if (callback === undefined) {
                callback = options;
                options = {};
            }
            options = options || {};
            options.eager = options.eager || 1;

            var t = this.$table,
                sql = DBA.selectByPK(t, this, options.eager);

            con.query({sql: sql, nestTables: true}, [pkval], function (err, result) {
                if (err) {
                    throw err;
                }

                var i,
                    max = result.length,
                    keys,
                    j,
                    jmax,
                    k,
                    result_ex,
                    entity;

                // @todo _typeCast is leaking!

                if (max === 1) {
                    norm.verbose("Entity::$get", result);
                    result = result[0];
                    entity = this.$from(result, true/*nested*/, true/*root*/, con); // jshint ignore:line
                } else if (max > 1) {
                    result_ex = {
                        root: result[0].root, // root wont change
                    };
                    keys = Object.keys(result[0]);
                    keys.splice(keys.indexOf("root"), 1);

                    jmax = keys.length;
                    for (i = 0; i < max; ++i) {
                        for (j = 0; j < jmax; ++j) {
                            k = keys[j];
                            result_ex[k] = result_ex[k] || [];
                            // do not add nulls
                            //var check = Array.unique(object.values(result[i][k]));
                            //if (!(check.length === 1 && check[0] === null)) {
                                result_ex[k].push(result[i][k]);
                            //}
                        }
                    }
                    for (j = 0; j < jmax; ++j) {
                        result_ex[k] = array.unique(result_ex[k]);
                    }

                    norm.verbose("Entity::$get", result_ex);

                    entity = this.$from(result_ex, true/*nested*/, true/*root*/, con); // jshint ignore:line
                }

                callback && callback(err, result ? entity : result, result);
            }.bind(this));
        },
        // internal
        "static $createTable": function () {
            var table_data = this.$table,
                fields = table_data.fields,
                create,
                alters = [],
                i,
                max,
                structure,
                sqlf = [],
                sqlk = [],
                rel,
                tableName = table_data.tableName,
                prefix = table_data.prefix;

            create = "CREATE TABLE `" + tableName + "` (\n";
            for (i = 0, max = fields.length; i < max; ++i) {
                structure = table_data.structure[fields[i]];

                sqlf.push(export_type(prefix, fields[i], structure, table_data.primaryKey));

                //fields[i]
            }

            sqlk.push("    PRIMARY KEY ( " + prefix + table_data.primaryKey + " )");

            for (i = 0, max = table_data.relations.length; i < max; ++i) {
                rel = table_data.relations[i];
                rel.creation(this, sqlk, alters);
            }

            array.combine(sqlf, sqlk);

            create = create + sqlf.join(",\n") + "\n) ENGINE = InnoDB;\n";

            return [create, alters];
        },
        "static $hasOne": function (target_entity, options) {
            // edge cases
            // relation to myself, and canBeNull is true

            Relation.OneToOne(this, target_entity, options);
            return this;
        },
        "static $hasMany": function (target_entity, options) {
            Relation.OneToMany(this, target_entity, options);
            return this;
        }
    });

}());