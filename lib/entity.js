    ...
    __class("Norm/Entity", {
        ...
        "static $from": function (result, nested, root) {
            ...
            for (i = 0, max = t.relations.length; i < max; ++i) {
                ...
                case "OneToMany":
                    ...
                    break;
                case "OneToOne":
                    if (relation.isRoot(this)) {
                        if (Array.isArray(result[relation.property])) {
                            result[relation.property] = result[relation.property][0];
                        }
                        entity[relation.property] =  relation.reference.$from(result[relation.property]);
                    } else {
                        if (Array.isArray(result[relation.property])) {
                            result[relation.refProperty] = result[relation.refProperty][0];
                        }
                        entity[relation.refProperty] =  relation.root.$from(result[relation.refProperty]);
                    }
                    break;
                }
            }
        ...
        "static $createTable": function () {
            ...
            for (i = 0, max = table_data.relations.length; i < max; ++i) {
                ...
                ttable_data = rel.reference.$table;
                //console.log("************");
                ...
                if (rel.owner === this) {
                    ...
                    case "ManyToOne":
                        break;
                    case "OneToOne":
                        sqlk.push("    UNIQUE INDEX `unique_" + tableName + "_" + rel.foreignKey + "` (`" + prefix + rel.foreignKey + "`)");
                        break;
                    }

                    //
                    alters.push("ALTER TABLE `" + tableName +
                        "` ADD CONSTRAINT `fk_" + tableName + "_" + rel.foreignKey + "` FOREIGN KEY (`" + prefix + rel.foreignKey + "`)" +
        ...
        "static $hasOne": function (target_entity, options) {
            ...
            var rel = Relation.OneToOne(this, target_entity, options);

            this.$table.relations.push(rel);
        ...
        "static $hasMany": function (target_entity, options) {
            var rel = Relation.OneToMany(this, target_entity, options);
