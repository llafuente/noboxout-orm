(function () {
    ...
    Norm = {
        ...
        logQuery: function(query, args) {
            var log;
            if ("string" === typeof query) {
                log = query + " " + JSON.stringify(args);
            } else {
                log = JSON.stringify(query) + " " + JSON.stringify(args);
            }

            util.print(new Date().toISOString().slice(0, 19).replace('T', ' '), "[qry]", __callee, " ", log.blue, "\n");
        },
        verbose: function() {
