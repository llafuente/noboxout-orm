(function () {
    ...
    Norm = __class("norm/norm", {
        ...
        "static verbose": function(input) {
            console.log(new Date().toISOString().slice(0, 19).replace('T', ' '), "[vrb]", __callee, input.cyanBG);
        },
        "static debug": function(input) {
            console.log(new Date().toISOString().slice(0, 19).replace('T', ' '), "[dbg]", __callee, input.blackBG);
        },
        "static warning": function(input) {
            console.log("w", new Date().toISOString().slice(0, 19).replace('T', ' '), "[wrn]", __callee, input.yellowBG);
        },
        "static error": function(input) {
            util.error("e", new Date().toISOString().slice(0, 19).replace('T', ' '), "[err]", __callee, input.redBG);
        }
