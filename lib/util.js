(function () {
    ...
    function parse_entity(Entity, result, nested, root, con) {
        ...
        if (ret) {
            ...
            for (i = 0, max = $t.relations.length; i < max; ++i) {
                ...
                norm.debug("parse_entity", rel.type);
                norm.debug("is-root", Entity.$class, rel.isRoot(Entity));
    ...
    module.exports = {
        ...
        storeRelations: function storeRelations (entity, relations, obj, con, callback) {
            var relation,
                i,
                max,
                retry = function () {
                    storeRelations(entity, relations, obj, con, callback);
                };

            for (i = 0, max = relations.length; i < max; ++i) {
                relation = relations[i];
                norm.err("hasForeignKey", entity.$class, relation.hasForeignKey(entity));
        ...
        storeEntity: function(entity, con, callback) {
            ...
            var table_data = entity.$self.$table,
                ...
                //obj = object.extract(entity, table_data.columns);
                obj = entity.$changes();
            ...
            this.storeRelations(entity, relations, obj, con, function() {
