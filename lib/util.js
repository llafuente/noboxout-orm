(function () {
    ...
    function parse_entity(Entity, result, nested, root, con) {
        ...
        norm.verbose("root parsed!!!" + inspect(ret));
    ...
    module.exports = {
        ...
        aggregateNested: function (entity, result) {
            ...
            for (i = 0; i < max; ++i) {
                ...
                norm.debug(inspect([pk, id, result[i].root, result[i].root[pk], id_to_idx]));
                if (!id) {
                    aggres.push({root: result[i].root});
                    id = id_to_idx[result[i].root[pk]] = aggres.length -1;
                }

                norm.debug(inspect(id));
        ...
        parseEntity: function (entity, con, result) {
            ...
            norm.debug("parsedEntity" + inspect(ret));
            norm.debug("******************");
